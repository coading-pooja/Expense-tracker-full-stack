  ...
  try {
    ...
    if (existingUser) {
      function generateAccessToken(id, name, isPremiumuser) {
        ...
        const token = jwt.sign({ id, name, isPremiumuser }, secretKey);
    ...
    bcrypt.compare(loginDetails.loginPassword, existingUser.password, (err, result) => {
      ...
      if (result==true) {
          ...
          res.status(200).json({ message: 'Logged in Sucessfully',token: generateAccessToken(existingUser.id, existingUser.name,existingUser.isPremiumuser)});
      }
      else {
